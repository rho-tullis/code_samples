/*-------------------------------------------------------------------*/
/*  Adapted from: subset_and_report.sas                              */
/*  Source repo:  rho-tullis/code_samples                            */
/*                                                                    */
/*  Jenner compatibility bundle -- the run_means and run_per_year     */
/*  macros below are reproduced verbatim from the source script,     */
/*  including the CALL EXECUTE trick that generates one %run_means   */
/*  call per chronic-condition variable found in the PDE data. The   */
/*  "source" libname is retargeted to WORK (autoexec.sas builds a    */
/*  small synthetic bene + PDE dataset there, in the field layout    */
/*  documented in this repo's DESynPUF_*_READIN scripts), and only   */
/*  the one year the mock data covers (2008) is run -- the original  */
/*  calls all three years (2008-2010) against a real multi-year      */
/*  CMS DE-SynPUF extract.                                           */
/*-------------------------------------------------------------------*/
